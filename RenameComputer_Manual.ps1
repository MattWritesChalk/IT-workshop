...
# --- Manual intervention: prompt for the computer name instead of computing it ---
do {
    $newName = (Read-Host "Enter the computer name").Trim()
    $valid = $true
    ...
    elseif ($newName.Length -gt 15) {
        Write-Warning "Name must be 15 characters or fewer (NetBIOS limit). You entered $($newName.Length)."
        $valid = $false
    }
    elseif ($newName -notmatch '^[A-Za-z0-9-]+$') {
        Write-Warning "Name may only contain letters, numbers, and hyphens."
        $valid = $false
    }
    elseif ($newName -match '^\d+$') {
        Write-Warning "Name cannot be all numbers."
        $valid = $false
    }
} until ($valid)

Write-Host "Using computer name: $newName" -ForegroundColor Green
